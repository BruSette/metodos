...
void main(){
  ...
  double vresultadonewton[n+1],vresultadolagrange[n+1],copvaloresX[n];
	copvaloresX[0] = vr[0] * (-1);
	vresultadonewton[0] = 0;
  vresultadolagrange[0] = 0;
	for (i = 1; i < n; i++) {
		copvaloresX[i] = valoresX[i] * (-1);
		vresultadonewton[i] = 0;
    vresultadolagrange[i] = 0;
		if (i == n-1){
      vresultadolagrange[i+1] = 0;
  ...
  printf("Polinômio pelo método de Lagrange: \n");
  apresentar(vresultadolagrange,n);
	newton(copvaloresX,vr,valoresY, vresultadonewton, n);
  printf("Polinômio pelo método de Newton: \n");
  apresentar(vresultadonewton,n);
  printf("Função com interpolação trigonométrica: \n");
